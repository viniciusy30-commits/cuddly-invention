        ...
        when (ev.actionMasked) {
            ...
            MotionEvent.ACTION_MOVE -> {
                if (!isPanning && ev.pointerCount == 1 &&
                    ...
                    // before touchSlop was exceeded, so its own page (the
                    // HTML document) may have already started scrolling
                    // internally. Cancel that gesture on the child explicitly,
                    // otherwise the page's own scroll offset and this host's
                    // pan offset both end up active at once: symptom is
                    // "scroll down reveals more, scroll back up breaks".
                    cancelChildTouch(ev)
                }
            }
