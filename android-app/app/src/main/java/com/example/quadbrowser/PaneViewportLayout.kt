package com.example.quadbrowser
    ...
    /**
    * Measures the child on a stable surface while fitting it into the visible grid cell.
    * The transform is applied to the child container rather than Canvas/WebView drawing,
    * so WebView scroll and hardware-composited content keep the correct clipping bounds.
    */
    class PaneViewportLayout @JvmOverloads constructor(
      ...
      override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
          ...
          val offsetX = (measuredWidth - width * surfaceScale).coerceAtLeast(0f) / 2f
          val offsetY = (measuredHeight - height * surfaceScale).coerceAtLeast(0f) / 2f
