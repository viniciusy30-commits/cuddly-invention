...
object AccessControlGate {
     ...
     */
    private fun registerPendingDeviceViaRest(deviceId: String, modelLabel: String): Boolean {
        return try {
            val url = URL(
    ...
    fun listen(context: Context, onDecision: (AccessDecision) -> Unit): ListenerRegistration {
        ...
        // TEMPORARY diagnostic: force a direct server read (bypassing the
        // realtime listener/cache entirely) right at startup. Unlike
        // addSnapshotListener — which can silently sit on stale/empty cache
        // data forever if the underlying gRPC stream never connects — a
        // Source.SERVER get() has no cache to fall back to, so it MUST
        // either succeed or fail with a real, reportable error (timeout,
        // host unreachable, etc). This tells us definitively whether the
        // device's connection to Firestore's backend works at all, instead
        // of guessing from a listener that just never fires.
        val mainHandler = android.os.Handler(android.os.Looper.getMainLooper())
        val diagnosticTimeoutMs = 12_000L
        var diagnosticResolved = false
        val diagnosticTimeoutRunnable = Runnable {
            if (!diagnosticResolved) {
                ...
                onDecision(
                    AccessDecision(
                        false,
                        "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                        debugError = "v6-rest-fallback | deviceId=$deviceId | TIMEOUT: a leitura direta do servidor não respondeu em 12s. O SDK do Firestore não está conseguindo conectar nesta rede/aparelho (nem sucesso, nem erro do servidor)."
                    )
                )
            }
        }
        mainHandler.postDelayed(diagnosticTimeoutRunnable, diagnosticTimeoutMs)
        docRef.get(com.google.firebase.firestore.Source.SERVER)
            .addOnSuccessListener {
                if (!diagnosticResolved) {
                    diagnosticResolved = true
                    mainHandler.removeCallbacks(diagnosticTimeoutRunnable)
                }
            }
                ...
                if (!diagnosticResolved) {
                    ...
                    onDecision(
                        AccessDecision(
                            false,
                            "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                            debugError = "v6-rest-fallback | deviceId=$deviceId | ERRO real do servidor: ${e.javaClass.simpleName}: ${e.message}"
                        )
                    )
                }
            }
        ...
        return docRef.addSnapshotListener { snapshot, error ->
            ...
            if (snapshot == null || !snapshot.exists()) {
                onDecision(
                    AccessDecision(
                        false,
                        "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                        // TEMPORARY: always visible, even before registration
                        // is attempted, so we can confirm (a) this build is
                        // actually the one running, and (b) exactly which
                        // deviceId this install is using — to check by hand
                        // against the Firebase console.
                        debugError = "v6-rest-fallback | deviceId=$deviceId | " +
                            (if (registered) "aguardando confirmação do registro..." else "ainda não registrado")
                    )
                )
                // First time this install has ever been seen: register it as
                ...
                if (!registered) {
                    ...
                    //
                    // IMPORTANT: this no longer uses MainScope().launch. A
                    // coroutine launched on MainScope is tied to nothing in
                    // particular and is NOT automatically cancelled by
                    // Activity recreation — but to rule out every possible
                    // silent-cancellation path while tracking down why
                    // devices were never reaching Firestore, this now uses
                    // Firebase's own Task callbacks directly (addOnSuccess/
                    // addOnFailureListener), which run independently of any
                    // coroutine scope and cannot be silently dropped.
                    ...
                    onDecision(
                        AccessDecision(
                            false,
                            "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                            debugError = "v6-rest-fallback | deviceId=$deviceId | enviando registro (SDK)..."
                        )
                    )

                    // FIX: race the normal SDK write against a REST fallback.
                    // On networks/devices where the SDK's gRPC channel hangs
                    // silently, the SDK call below never calls either
                    // listener — so a plain timer, independent of it, is
                    // used to fire the REST fallback (see
                    // registerPendingDeviceViaRest above) if the SDK hasn't
                    // confirmed within 6s. Whichever path wins marks
                    // restFallbackHandled so the other path becomes a no-op
                    // when/if it eventually does resolve.
                    ...
                    val restFallbackRunnable = Runnable {
                        ...
                        onDecision(
                            AccessDecision(
                                false,
                                "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                                debugError = "v6-rest-fallback | deviceId=$deviceId | SDK não respondeu em 6s, tentando registro via REST (contorna gRPC)..."
                            )
                        )
                        Thread {
                            val restSuccess = registerPendingDeviceViaRest(deviceId, modelLabel)
                                ...
                                if (restSuccess) {
                                    onDecision(
                                        AccessDecision(
                                            false,
                                            "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                                            debugError = "v6-rest-fallback | deviceId=$deviceId | registro confirmado via REST. Aguardando o painel/listener atualizar..."
                                        )
                                    )
                                } else {
                                    registered = false
                                    onDecision(
                                        AccessDecision(
                                            false,
                                            "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                                            debugError = "v6-rest-fallback | deviceId=$deviceId | REST também falhou. Verifique a conexão com a internet deste aparelho."
                                        )
                                    )
                                }
                            }
                        ...
                        .addOnSuccessListener {
                            ...
                            onDecision(
                                AccessDecision(
                                    false,
                                    "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                                    debugError = "v6-rest-fallback | deviceId=$deviceId | registro confirmado pelo Firestore (SDK). Aguardando o painel/listener atualizar..."
                                )
                            )
                            // TEMPORARY: fingerprint/IP capture disabled while
                            // tracking down why registration was hanging — to
                            // rule out whether these extra fields/calls were
                            // the cause. Re-enable once confirmed unrelated.
                        }
                        .addOnFailureListener { e ->
                            ...
                            onDecision(
                                AccessDecision(
                                    false,
                                    "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                                    debugError = "v6-rest-fallback | deviceId=$deviceId | registro (SDK): ${e.javaClass.simpleName}: ${e.message}"
                                )
                            )
                        }
                }
            ...
            // Best-effort heartbeat; keeps deviceModel/lastSeen current.
            // Failure here should never block access.
            // TEMPORARY: IP capture disabled here too while ruling out
            // whether it was related to the registration hang.
            MainScope().launch {
                val heartbeat = hashMapOf<String, Any>(
                    "lastSeen" to com.google.firebase.firestore.FieldValue.serverTimestamp(),
                    "deviceModel" to modelLabel,
