            ...
            if (!diagnosticResolved) {
                ...
                onDecision(
                    AccessDecision(
                        false,
                        "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                        debugError = "v4-diag | deviceId=$deviceId | TIMEOUT: a leitura direta do servidor não respondeu em 12s. O SDK do Firestore não está conseguindo conectar nesta rede/aparelho (nem sucesso, nem erro do servidor)."
                    )
                )
            }
        }
                ...
                if (!diagnosticResolved) {
                    ...
                    onDecision(
                        AccessDecision(
                            false,
                            "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                            debugError = "v4-diag | deviceId=$deviceId | ERRO real do servidor: ${e.javaClass.simpleName}: ${e.message}"
                        )
                    )
                }
            }
            ...
            if (snapshot == null || !snapshot.exists()) {
                onDecision(
                    AccessDecision(
                        ...
                        debugError = "v3 | deviceId=$deviceId | " +
                ...
                if (!registered) {
                    ...
                    onDecision(
                        AccessDecision(
                            false,
                            "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                            debugError = "v3 | deviceId=$deviceId | enviando registro..."
                        )
                    )
                        ...
                        .addOnSuccessListener {
                            onDecision(
                                AccessDecision(
                                    false,
                                    "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                                    debugError = "v3 | deviceId=$deviceId | registro confirmado pelo Firestore (onSuccess). Aguardando o painel/listener atualizar..."
                                )
                            )
                        ...
                        .addOnFailureListener { e ->
                            ...
                            onDecision(
                                AccessDecision(
                                    false,
                                    "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                                    debugError = "v3 | deviceId=$deviceId | registro: ${e.javaClass.simpleName}: ${e.message}"
                                )
                            )
                        }
                }
