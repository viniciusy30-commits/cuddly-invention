            ...
            if (!diagnosticResolved) {
                ...
                onDecision(
                    AccessDecision(
                        false,
                        "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                        debugError = "v5-longpoll | deviceId=$deviceId | TIMEOUT: mesmo com long-polling forçado, a leitura direta do servidor não respondeu em 12s. Provavelmente é a rede/Wi-Fi bloqueando a conexão, não mais o SDK."
                    )
                )
            }
        }
                ...
                if (!diagnosticResolved) {
                    ...
                    onDecision(
                        AccessDecision(
                            false,
                            "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                            debugError = "v5-longpoll | deviceId=$deviceId | ERRO real do servidor: ${e.javaClass.simpleName}: ${e.message}"
                        )
                    )
                }
            }
            ...
            if (snapshot == null || !snapshot.exists()) {
                onDecision(
                    AccessDecision(
                        ...
                        debugError = "v5-longpoll | deviceId=$deviceId | " +
                ...
                if (!registered) {
                    ...
                    onDecision(
                        AccessDecision(
                            false,
                            "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                            debugError = "v5-longpoll | deviceId=$deviceId | enviando registro..."
                        )
                    )
                        ...
                        .addOnSuccessListener {
                            onDecision(
                                AccessDecision(
                                    false,
                                    "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                                    debugError = "v5-longpoll | deviceId=$deviceId | registro confirmado pelo Firestore (onSuccess). Aguardando o painel/listener atualizar..."
                                )
                            )
                        ...
                        .addOnFailureListener { e ->
                            ...
                            onDecision(
                                AccessDecision(
                                    false,
                                    "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                                    debugError = "v5-longpoll | deviceId=$deviceId | registro: ${e.javaClass.simpleName}: ${e.message}"
                                )
                            )
                        }
                }
