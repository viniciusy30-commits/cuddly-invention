            ...
            if (!diagnosticResolved) {
                ...
                onDecision(
                    AccessDecision(
                        false,
                        "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                        debugError = "v7-rest-detail | deviceId=$deviceId | TIMEOUT: a leitura direta do servidor não respondeu em 12s. O SDK do Firestore não está conseguindo conectar nesta rede/aparelho (nem sucesso, nem erro do servidor)."
                    )
                )
            }
        }
                ...
                if (!diagnosticResolved) {
                    ...
                    onDecision(
                        AccessDecision(
                            false,
                            "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                            debugError = "v7-rest-detail | deviceId=$deviceId | ERRO real do servidor: ${e.javaClass.simpleName}: ${e.message}"
                        )
                    )
                }
            }
            ...
            if (snapshot == null || !snapshot.exists()) {
                onDecision(
                    AccessDecision(
                        ...
                        debugError = "v7-rest-detail | deviceId=$deviceId | " +
                ...
                if (!registered) {
                    ...
                    onDecision(
                        AccessDecision(
                            false,
                            "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                            debugError = "v7-rest-detail | deviceId=$deviceId | enviando registro (SDK)..."
                        )
                    )
                    ...
                    val restFallbackRunnable = Runnable {
                        ...
                        onDecision(
                            AccessDecision(
                                false,
                                "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                                debugError = "v7-rest-detail | deviceId=$deviceId | SDK não respondeu em 6s, tentando registro via REST (contorna gRPC)..."
                            )
                        )
                        Thread {
                            val (restSuccess, restError) = registerPendingDeviceViaRest(deviceId, modelLabel)
                                ...
                                if (restSuccess) {
                                    onDecision(
                                        AccessDecision(
                                            false,
                                            "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                                            debugError = "v7-rest-detail | deviceId=$deviceId | registro confirmado via REST. Aguardando o painel/listener atualizar..."
                                        )
                                    )
                                } else {
                                    registered = false
                                    onDecision(
                                        AccessDecision(
                                            false,
                                            "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                                            debugError = "v7-rest-detail | deviceId=$deviceId | REST falhou: $restError"
                                        )
                                    )
                                }
                            }
                        ...
                        .addOnSuccessListener {
                            ...
                            onDecision(
                                AccessDecision(
                                    false,
                                    "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                                    debugError = "v7-rest-detail | deviceId=$deviceId | registro confirmado pelo Firestore (SDK). Aguardando o painel/listener atualizar..."
                                )
                            )
                        ...
                        .addOnFailureListener { e ->
                            ...
                            onDecision(
                                AccessDecision(
                                    false,
                                    "Este dispositivo ainda não foi aprovado. Aguarde a liberação do administrador.",
                                    debugError = "v7-rest-detail | deviceId=$deviceId | registro (SDK): ${e.javaClass.simpleName}: ${e.message}"
                                )
                            )
                        }
                }
