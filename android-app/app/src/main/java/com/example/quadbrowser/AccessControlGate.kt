        ...
        return docRef.addSnapshotListener { snapshot, error ->
            ...
            if (snapshot == null || !snapshot.exists()) {
                ...
                if (!registered) {
                    ...
                    // claim or the IP lookup succeeding. Earlier, all of that
                    // ran before the device doc was written and its failure
                    // was silently swallowed (no try/catch, no .await()), so
                    // any hiccup with the new hardwareFingerprints rule could
                    // keep a device from ever showing up in the panel with no
                    // visible error at all. Now the device always registers
                    // first; the IP and fingerprint are attached right after,
                    // best-effort, and never block the initial registration.
                    ...
                    MainScope().launch {
                        try {
                            ...
                            if (extra.isNotEmpty()) docRef.set(extra, SetOptions.merge())
                        } catch (_: Exception) {
