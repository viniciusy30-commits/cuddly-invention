...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // TEMPORARY diagnostic screen — if the previous run of the app
        // crashed, show the saved crash text full-screen right now, before
        // anything else (including the access gate), so it can be read and
        // photographed. Tapping "Continuar" dismisses it and proceeds
        // normally. Safe to remove once the root cause is found (see
        // QuadBrowserApplication's crash catcher).
        val crashLogFile = java.io.File(filesDir, QuadBrowserApplication.CRASH_LOG_FILE_NAME)
        if (crashLogFile.exists()) {
            val crashText = try { crashLogFile.readText() } catch (_: Exception) { "(falha ao ler o log)" }
            crashLogFile.delete()
            showCrashReportScreen(crashText)
            return
        }
...
row.addView(compactAction("P", R.string.auto_clicker_presets) { showPresetDialog(index) }, LinearLayout.LayoutParams(0, dp(32), 1f).apply { setMargins(0, dp(3), 0, 0) })
    ...
    /**
     * TEMPORARY diagnostic screen — shows the previous crash's full stack
     * trace with a copy button, then a "Continuar" button that restarts
     * onCreate normally (the crash file has already been deleted by the
     * caller, so it won't show again). Safe to remove once the root cause of
     * the registration issue is found.
     */
    private fun showCrashReportScreen(crashText: String) {
        ...
        val title = TextView(this).apply {
            text = "O app fechou sozinho na última vez"
            textSize = 19f
            setTextColor(Color.WHITE)
        }
        val subtitle = TextView(this).apply {
            text = "Copie o texto abaixo e envie para análise."
            textSize = 14f
