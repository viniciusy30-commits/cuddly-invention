...
class MainActivity : AppCompatActivity() {
    ...
    private fun applyScaledPaneViewport(pane: BrowserPane) {
        ...
        // Always re-derive the reference from the current screen instead of
        // reusing whatever was captured once before: a stale reference is
        // what let cells in 2x2 end up letterboxed or oddly sized instead of
        // filling their slot after a rotation or the very first layout pass.
        ...
        // Scale width and height independently so the page always fills its
        // own cell exactly, with no leftover letterboxed space — the cell's
        // size decides the page's size, not one shared uniform ratio.
        val scaleX = (hostWidth.toFloat() / referenceWidth.toFloat()).coerceIn(0.01f, 1f)
        val scaleY = (hostHeight.toFloat() / referenceHeight.toFloat()).coerceIn(0.01f, 1f)
        ...
        pane.gridScale = (scaleX + scaleY) / 2f
        pane.gridTransformApplied = pane.thumbnailHost.setSurfaceSize(referenceWidth, referenceHeight, scaleX, scaleY)
    }
