                ...
                when (event.actionMasked) {
                    ...
                    MotionEvent.ACTION_UP -> {
                        if (!dragging) {
                            ...
                            if (from >= 0 && pendingTarget >= 0 && from != pendingTarget) {
                                swapPanePositions(from, pendingTarget)
                            }
                        }
                        dragging = false
                        pendingTarget = -1
...
row.addView(compactAction("P", R.string.auto_clicker_presets) { showPresetDialog(index) }, LinearLayout.LayoutParams(0, dp(32), 1f).apply { setMargins(0, dp(3), 0, 0) })
    ...
    private fun buildAccessOverlay(reason: String): LinearLayout {
        val overlay = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            gravity = Gravity.CENTER
            setPadding(48, 48, 48, 48)
    ...
    private fun onAccessRevoked(reason: String) {
        ...
        val root = findViewById<FrameLayout>(android.R.id.content)
        val overlay = buildAccessOverlay(reason)
        root.addView(overlay, FrameLayout.LayoutParams(-1, -1))
        accessGateOverlay = overlay
    }
