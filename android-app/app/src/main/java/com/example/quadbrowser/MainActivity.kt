...
class MainActivity : AppCompatActivity() {
    ...
    private fun applyActualSizeWebViewViewport(pane: BrowserPane) {
        ...
        // size. Reapplying and dispatching two JS resize events for every
        // pane in that case makes sites such as pokeidle.io do avoidable
        ...
        pane.thumbnailHost.panEnabled = false
        pane.bodyViewportHost.panEnabled = false
        pane.gridScalePercent = null
    ...
    private fun applyGridWebViewViewport(pane: BrowserPane) {
        // v2 design: the WebView is always laid out at exactly the grid
        // cell's real on-screen size (see PaneViewportLayout) — no oversized
        // View surface, no scaleX/scaleY/translation, so there is nothing
        // left for Android's renderer to mis-cache in the bottom grid row.
        //
        // To show the *whole* fullscreen page shrunk down to fit the cell
        // (not a 1:1 crop), the shrink factor is computed here in Kotlin —
        // cellWidthPx / fullscreenWidthPx — and applied with
        // WebView.setInitialScale(percent), which is a real engine zoom, not
        // a View transform. Letting the WebView/CSS viewport pick this
        // automatically (initial-scale=1 + loadWithOverviewMode) was tried
        // first and was unreliable: the engine sometimes settled on a zoom
        // above 100% (observed engineScale=1.75 in the diagnostics overlay),
        // which crops most of the page instead of shrinking it. Computing
        // and forcing the exact percent removes that guesswork.
        ...
        pane.thumbnailHost.panEnabled = false
        pane.bodyViewportHost.panEnabled = true
        pane.gridScalePercent = null
        pane.gridTransformApplied = true
        pane.gridReferenceWidth = referenceWidth
        pane.gridReferenceHeight = referenceHeight
        pane.gridHostWidth = hostWidth
        pane.gridHostHeight = hostHeight
        // Fit-to-cell: the smaller of the two ratios keeps both width and
        // height of the fullscreen page inside the cell (letterboxed if the
        // aspect ratios differ), so nothing is cropped off either edge.
        val widthRatio = hostWidth.toFloat() / referenceWidth.toFloat()
        val heightRatio = hostHeight.toFloat() / referenceHeight.toFloat()
        val zoomPercent = (minOf(widthRatio, heightRatio) * 100f)
            .roundToInt()
            .coerceIn(10, 100)
        pane.gridScale = zoomPercent / 100f
        pane.webView.settings.useWideViewPort = true
        pane.webView.settings.loadWithOverviewMode = false
        restoreDefaultPageViewport(pane.webView)
        applyForcedWebViewZoom(pane, zoomPercent)
    }
    ...
    private fun thumbnailReferenceSize(): Pair<Int, Int> {
        ...
        // Only used as a "full size" reference to compute the grid cell's
        // zoom percentage against (see applyGridWebViewViewport). The
        // WebView itself is never laid out at this size — it always stays
        // at the grid cell's real, on-screen dimensions.
              ...
              marker.setOnTouchListener { view, event ->
                  ...
                  when (event.actionMasked) {
                      MotionEvent.ACTION_DOWN -> {
                          ...
                          startViewX = view.x
                          startViewY = view.y
                          // Converts a raw on-screen drag distance into the
                          // marker's own coordinate space, which is scaled
                          // to fit the reference (fullscreen) size into
                          // whatever the click layer's current real size is
                          // — the same ratio used to place the marker above.
                          gestureScale = (layerWidth / referenceWidth).takeIf { it > 0f } ?: 1f
                          moved = false
...
row.addView(compactAction("P", R.string.auto_clicker_presets) { showPresetDialog(index) }, LinearLayout.LayoutParams(0, dp(32), 1f).apply { setMargins(0, dp(3), 0, 0) })
    ...
    private fun logGridDiagnostics(trigger: String) {
        ...
        panes.forEachIndexed { index, pane ->
            ...
            lines.append("P").append(index + 1)
                ...
                .append(" pan=").append(pane.bodyViewportHost.panEnabled)
                .append(" zoom%=").append(pane.webViewZoomPercent)
                .append(" wvScroll=").append(pane.webView.scrollX).append(",").append(pane.webView.scrollY)
                .append(" engineScale=").append("%.2f".format(pane.webView.scale))
                .append("\n")
        }
    ...
    /**
     * Forces the WebView's real engine zoom to exactly [targetPercent],
     * regardless of what applyWebViewZoom last recorded. Used for the grid
     * cell's "shrink the whole fullscreen page to fit" zoom, which must be
     * an absolute, reliable value — not a relative zoomBy() against a
     * possibly stale bookkeeping field. Re-applies a second time shortly
     * after, because pages that call their own viewport/meta JS on load can
     * otherwise silently override the zoom right after it is set (this is
     * how a 100%+ engine zoom was observed even though the grid asked for a
     * shrink).
     */
    private fun applyForcedWebViewZoom(pane: BrowserPane, targetPercent: Int) {
        val target = targetPercent.coerceIn(10, 100)
        pane.webViewZoomPercent = target
        val apply = {
            pane.webView.setInitialScale(target)
        ...
        pane.webView.post(apply)
        pane.webView.postDelayed(apply, 300L)
    }
        ...
        webView.webViewClient = object : WebViewClient() {
            ...
            override fun onPageFinished(view: WebView, url: String) {
                ...
                panes.getOrNull(paneIndex)?.let { current ->
                    if (current.bodyViewportHost.panEnabled) {
                        restoreDefaultPageViewport(view)
                        applyForcedWebViewZoom(current, (current.gridScale * 100f).roundToInt().coerceIn(10, 100))
                    }
                }
