...
class MainActivity : AppCompatActivity() {
    ...
    private fun applyGridWebViewViewport(pane: BrowserPane) {
        ...
        // cell's real on-screen size (see PaneViewportLayout), and always at
        // 100% engine zoom — the site renders at its real size, cropped to
        // the cell, never shrunk to fit. Reaching the rest of the page is
        // done by dragging inside the cell, which now performs a real
        // WebView.scrollBy(): there is no oversized View surface being
        // scaled down anymore, so there is nothing left for Android's
        // renderer to mis-cache in the bottom grid row or anywhere else.
        val hostWidth = pane.bodyViewportHost.width
        val hostHeight = pane.bodyViewportHost.height
