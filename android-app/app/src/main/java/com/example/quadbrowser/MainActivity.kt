...
class MainActivity : AppCompatActivity() {
    ...
    companion object {
        ...
        // Grid thumbnails render at this fraction of the real fullscreen
        // resolution. Lower = lighter on CPU/GPU with 4 panes active at
        // once, at the cost of some sharpness in the small thumbnail (not
        // noticeable at this size). 1.0 would mean full native resolution.
        const val GRID_REFERENCE_SCALE = 0.6f
    }
    ...
    private fun applyGridWebViewViewport(pane: BrowserPane) {
        // The visual scaling is handled entirely by PaneViewportLayout
        // (View.scaleX/scaleY), so the WebView itself is always kept at the
        // same fullscreen viewport/zoom as in single-pane mode. This is what
        // makes the grid a true "zoomed out" copy instead of a page that
        // re-flows its own responsive layout for a tiny viewport.
    ...
    private fun refreshGridPaneThumbnails() {
        ...
        // Use a scaled-down copy of the fullscreen surface's aspect ratio as
        // the source image (not the literal full resolution). The page still
        // renders at "believes it's fullscreen" width/height for layout
        // purposes — this only lowers the actual pixel count Chromium has to
        // paint for a thumbnail that is visually small anyway, which matters
        // a lot with 4 of these rendering at once. GRID_REFERENCE_SCALE only
        // applies to the grid; fullscreen/paged mode is unaffected.
        val (fullSurfaceWidth, fullSurfaceHeight) = thumbnailReferenceSize()
        val surfaceWidth = (fullSurfaceWidth * GRID_REFERENCE_SCALE).roundToInt().coerceAtLeast(1)
        val surfaceHeight = (fullSurfaceHeight * GRID_REFERENCE_SCALE).roundToInt().coerceAtLeast(1)
