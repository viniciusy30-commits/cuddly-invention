...
class MainActivity : AppCompatActivity() {
    ...
    private data class BrowserPane(
        ...
        // Set whenever something outside applyGridWebViewViewport (fullscreen,
        // pager) touches this pane's surface/scale. Forces the next grid pass
        // to reapply for real instead of trusting a cache that no longer
        // matches what PaneViewportLayout is actually showing.
        var gridSurfaceStale: Boolean = true,
        var gridReferenceWidth: Int = 0,
        var gridReferenceHeight: Int = 0,
        var lastInjectedViewportHeightCss: Int = 0,
    ...
    private fun applyActualSizeWebViewViewport(pane: BrowserPane) {
        ...
        pane.gridSurfaceStale = true
        pane.gridReferenceWidth = hostWidth
        pane.gridReferenceHeight = hostHeight
        pane.gridHostWidth = hostWidth
        pane.gridHostHeight = hostHeight
        pane.gridScale = 1f
        // Full/actual size: the WebView fills real screen bounds 1:1, so
        // hardware compositing is safe again here.
    ...
    private fun applyFullscreenWebViewViewport(pane: BrowserPane) {
        ...
        pane.gridScalePercent = null
        pane.gridTransformApplied = true
        // The grid cache (host/reference size, scale) is still numerically
        // valid here — only the surface itself changed. Without this flag,
        // returning to the grid with the same cell size makes
        // applyGridWebViewViewport skip reapplying setSurfaceSize(), leaving
        // the page laid out at its fullscreen (reflowed) size but clipped
        // into the small cell: the "compressed" pane bug.
        pane.gridSurfaceStale = true
        // Same as applyActualSizeWebViewViewport: real 1:1 bounds again.
        pane.webView.setLayerType(View.LAYER_TYPE_HARDWARE, null)
        pane.webView.settings.useWideViewPort = true
        pane.webView.settings.loadWithOverviewMode = false
        pane.webView.setInitialScale(0)
    ...
    private fun applyGridWebViewViewport(pane: BrowserPane) {
        ...
        if (!pane.gridSurfaceStale &&
            ...
            pane.gridHostHeight == hostHeight &&
            pane.gridScale == 1f &&
            pane.bodyViewportHost.hasSurface
        ) return
        ...
        pane.gridScalePercent = null
        pane.gridTransformApplied = true
        pane.gridSurfaceStale = false
        pane.gridReferenceWidth = referenceWidth
        pane.gridReferenceHeight = referenceHeight
        pane.gridHostWidth = hostWidth
        pane.gridHostHeight = hostHeight
        pane.gridScale = 1f
        // The WebView here is measured/laid out far larger (the fullscreen
        // surface, e.g. 706x1305) than the small grid cell that actually
        // clips it via scaleX/scaleY + translation. Android's hardware
        // renderer keeps a display list/texture per View sized to its own
        // bounds, and when that oversized bounds is scaled down and placed
        // in the bottom row of the grid, the driver-side render cache for
        // that layer has been observed to go stale (showing an old/offset
        // frame of the page) in a way the top row does not exhibit — likely
        // because the top-row cell falls fully inside the GPU's default
        // render cache window and the bottom-row one does not. Software
        // layer type forces a plain bitmap redraw on every frame instead of
        // relying on that cache, trading a little battery for correctness
        // while the pane is small.
        pane.webView.setLayerType(View.LAYER_TYPE_SOFTWARE, null)
        pane.webView.settings.useWideViewPort = true
        pane.webView.settings.loadWithOverviewMode = false
        pane.webView.setInitialScale(0)
        val injectedHeightCss = gridViewportHeightCss(referenceHeight)
        restoreDefaultPageViewport(pane.webView, injectedHeightCss)
        pane.lastInjectedViewportHeightCss = injectedHeightCss
        applyWebViewZoom(pane, 100)
    }
...
row.addView(compactAction("P", R.string.auto_clicker_presets) { showPresetDialog(index) }, LinearLayout.LayoutParams(0, dp(32), 1f).apply { setMargins(0, dp(3), 0, 0) })
    ...
    private fun logGridDiagnostics(trigger: String) {
        ...
        panes.forEachIndexed { index, pane ->
            ...
            val cachedMatch = !pane.gridSurfaceStale &&
                pane.gridTransformApplied &&
                ...
                pane.gridScale == 1f &&
                pane.bodyViewportHost.hasSurface
            val body = pane.bodyViewportHost.getChildAt(0)
            lines.append("P").append(index + 1)
                ...
                .append(" stale=").append(pane.gridSurfaceStale)
                .append(if (cachedMatch) " SKIP" else " APPLY")
                .append(" wv=").append(pane.webView.width).append("x").append(pane.webView.height)
                .append(" cssH=").append(pane.lastInjectedViewportHeightCss)
                .append(" scale=").append("%.2f".format(body?.scaleX ?: -1f)).append("x").append("%.2f".format(body?.scaleY ?: -1f))
                .append(" tr=").append(body?.translationX?.toInt() ?: -1).append(",").append(body?.translationY?.toInt() ?: -1)
                ...
                .append(" layer=").append(if (pane.webView.layerType == View.LAYER_TYPE_SOFTWARE) "SW" else "HW")
                .append("\n")
        }
            ...
            override fun onPageFinished(view: WebView, url: String) {
                ...
                panes.getOrNull(paneIndex)?.let { current ->
                    if (current.bodyViewportHost.hasSurface && current.gridReferenceHeight > 1) {
                        val h = gridViewportHeightCss(current.gridReferenceHeight)
                        restoreDefaultPageViewport(view, h)
                        current.lastInjectedViewportHeightCss = h
                    }
                }
