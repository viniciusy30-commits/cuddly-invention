...
class MainActivity : AppCompatActivity() {
    ...
    private var paneOrder = mutableListOf(0, 1, 2, 3, 4, 5, 6, 7)
    private val defaultPaneNames = (1..8).map { "Instância $it" }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        pagerDots = listOf(
            ...
            findViewById(R.id.pager_dot_5),
            findViewById(R.id.pager_dot_6),
            findViewById(R.id.pager_dot_7),
            findViewById(R.id.pager_dot_8),
        )
        ...
        val definitions = listOf(
            ...
            PaneDefinition(R.id.pane_5, R.id.avatar_5, R.id.pane_title_5, R.id.pane_drag_handle_5, R.id.pane_subtitle_5, R.id.navigate_5, R.id.reload_5, R.id.fullscreen_5, R.id.auto_click_5, R.id.close_5, R.id.empty_state_5, R.id.reopen_5, R.id.webview_5, "webview5"),
            PaneDefinition(R.id.pane_6, R.id.avatar_6, R.id.pane_title_6, R.id.pane_drag_handle_6, R.id.pane_subtitle_6, R.id.navigate_6, R.id.reload_6, R.id.fullscreen_6, R.id.auto_click_6, R.id.close_6, R.id.empty_state_6, R.id.reopen_6, R.id.webview_6, "webview6"),
            PaneDefinition(R.id.pane_7, R.id.avatar_7, R.id.pane_title_7, R.id.pane_drag_handle_7, R.id.pane_subtitle_7, R.id.navigate_7, R.id.reload_7, R.id.fullscreen_7, R.id.auto_click_7, R.id.close_7, R.id.empty_state_7, R.id.reopen_7, R.id.webview_7, "webview7"),
            PaneDefinition(R.id.pane_8, R.id.avatar_8, R.id.pane_title_8, R.id.pane_drag_handle_8, R.id.pane_subtitle_8, R.id.navigate_8, R.id.reload_8, R.id.fullscreen_8, R.id.auto_click_8, R.id.close_8, R.id.empty_state_8, R.id.reopen_8, R.id.webview_8, "webview8"),
        )
...
row.addView(compactAction("P", R.string.auto_clicker_presets) { showPresetDialog(index) }, LinearLayout.LayoutParams(0, dp(32), 1f).apply { setMargins(0, dp(3), 0, 0) })
    ...
    private fun beginAccessGate() {
        ...
        lifecycleScope.launch {
            ...
            if (decision.allowed) {
                ...
                showAccessDeniedOverlay(overlay, progress, title, message, decision.reason)
            }
        }
    }

    /** Shows the blocking "access denied" screen, replacing whatever was on the overlay before. */
    private fun showAccessDeniedOverlay(overlay: LinearLayout, progress: ProgressBar, title: TextView, message: TextView, reason: String) {
        ...
        accessListenerRegistration = AccessControlGate.listen(applicationContext) { decision ->
            runOnUiThread {
                if (decision.allowed) {
                    ...
                    onAccessRevoked(decision.reason)
                }
            }
        }
    }
     ...
     * app is already open. Immediately shows the blocking screen over
     * everything, regardless of what the user was doing.
     */
    private fun onAccessRevoked(reason: String) {
        accessCountdownHandler.removeCallbacks(accessCountdownRunnable)
        stopAccessCountdownBlink()
        accessTimeRemainingLabel.visibility = View.GONE
        if (accessGateOverlay != null) return // already showing
        val root = findViewById<FrameLayout>(android.R.id.content)
        val overlay = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            gravity = Gravity.CENTER
    ...
    private fun loadPaneOrder(savedInstanceState: Bundle?): MutableList<Int> {
        val defaultOrder = (0 until panes.size.coerceAtLeast(8)).toList()
